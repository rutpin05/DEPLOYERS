...
# ifdef GLG_GTK
#  define GLG_EVENT_BASE  ( GDK_EVENT_LAST + 2000 )
# else
...
# define GLG_LONG_MAX   LLONG_MAX
# define GLG_LONG_MIN   LLONG_MIN
# define GLG_ULONG_MAX  ULLONG_MAX
#else
  typedef long GlgLong;
  typedef unsigned long GlgULong;
# define GLG_LONG_MAX   LONG_MAX
# define GLG_LONG_MIN   LONG_MIN
# define GLG_ULONG_MAX  LONG_MAX
#endif
...
/* Line cap style */
typedef enum _GlgLineCap
{
   GLG_AUTO_LINE_CAP = 0,
   GLG_BUTT_LINE_CAP = 1,
   GLG_ROUND_LINE_CAP = 2,
...
#define GLG_FIXED_BOX        GLG_FIXED_BOX_TEXT
...
#define GLG_SCREEN_OFFSET_XF GLG_PIXEL_OFFSET_XF
...
#ifdef MS_WINDOWS
   , GLG_NON_CLIENT_AREA_EVENT_MOD = 100
#endif
} GlgModifierType;
...
typedef enum _GlgPositionType
{
   GLG_UNDEFINED_POSITION_TYPE = 0,
   GLG_FIRST,
   GLG_LAST,
   GLG_START,
   GLG_END
...
typedef enum _GlgBoundType
{
   GLG_UNBOUND = 0,
   GLG_BOUND,           /* Bound: uses value defined in the drawing. */
   GLG_INVERSE_BOUND    /* Inverse bound: inherits value from the template. */
} GlgBoundType;
...
typedef enum _GlgCallbackType
{
   ...
   GLG_TRACE_CB,
   GLG_TRACE2_CB,
   GLG_MOTIF_INPUT_CB,   /* X11 widget */
   GLG_MOTIF_SELECT_CB,  /* X11 widget */
   GLG_CUSTOM_DRAW_CB,
   GLG_HIERARCHY_CB,
   GLG_H_CB,             /* GTK GLG widget */
   GLG_V_CB              /* GTK GLG widget */
} GlgCallbackType;
...
typedef enum _GlgDoubleBufferingType
{
   GLG_DB_OFF = 0,
   GLG_DB_ON,
   GLG_DB_FORCED
...
/* Scaling adjustment type for the pixel offset and screen scaling xforms. */
typedef enum _GlgScaleAdjustmentType
{
   GLG_WORLD_SCALING = 0,  /* Do not apply any adjustments. */
...
typedef enum _GlgControlPointType
{
   ...
   GLG_CONTROL_POINTS = 1,
   GLG_CONTROL_AND_ATTACHMENT_POINTS = 17
} GlgControlPointType;
...
typedef enum _GlgScrollbarType
{
   GLG_DEFAULT_SCROLLBAR = 0,
   GLG_NATIVE_SCROLLBAR,
   GLG_VIEWPORT_SCROLLBAR,
   GLG_LIGHT_VIEWPORT_SCROLLBAR
} GlgScrollbarType;

/* Chart CrossHair flags. */
...
/* Chart DrawGrid flags. */
typedef enum _GlgGridType
{
   GLG_NO_GRID       = 0,
   GLG_GRID_X        = 1,   /* Major X grid */
   GLG_GRID_Y        = 2,   /* Major Y grid */
   GLG_GRID_x        = 4,   /* Minor X grid */
   GLG_GRID_y        = 8,   /* Minor Y grid */
} GlgGridType;
...
/* Chart Level Line and Time Line drawing order types. */
typedef enum _GlgLevelDrawOrder
{
   GLG_IN_FRONT_OF_PLOT = 0,
   GLG_BEHIND_PLOT,
   GLG_SPLIT_DRAW     /* AREA level types only */
} GlgLevelDrawOrder;

typedef enum _GlgElementDrawType
{
   GLG_DONT_CREATE = -1,
   GLG_DONT_DRAW = 0,
   GLG_IN_BACKGROUND,
...
/* Level line and Time line types. */
typedef enum _GlgLevelType
{
   GLG_LINE_LEVEL = 0,
   GLG_AREA_LEVEL
} GlgLevelType;
...
typedef enum _GlgChartAutoScale
{
   GLG_SCALE_DISABLED = 0,
   GLG_SCALE_UP,
...
typedef enum _GlgChartCacheUse
{   
   GLG_NO_CHART_CACHE = 0,
   GLG_USE_CHART_CACHE = 1,
   GLG_USE_PREALLOCATED_CHART_CACHE = 3
} GlgChartCacheUse;
...
typedef enum _GlgLayoutType
{
   ...
   GLG_ALIGN_LEFT,          /* Align left edges */
   GLG_ALIGN_RIGHT,         /* Align right edges */
   GLG_ALIGN_HCENTER,       /* Align horizontal centers
                               (center on vertical axis) */
   GLG_ALIGN_TOP,           /* Align top edges */
   GLG_ALIGN_BOTTOM,        /* Align bottom edges */
   GLG_ALIGN_VCENTER,       /* Align vertical centers 
...
typedef enum _GlgCompatibilityMode
{
   ...
   GLG_PRE_4_2,        /* Changed mapping of negative viewport coordinates. */
   GLG_PRE_4_3,        /* Reset Global Flag to LOCAL when binding. */
...
typedef enum _GlgCursorType
{
   GLG_DEFAULT_CURSOR   = -1,
   GLG_CROSSHAIR_CURSOR = 0,
   GLG_WAIT_CURSOR,
   GLG_HAND_CURSOR,
   GLG_MOVE_CURSOR
} GlgCursorType;
...
*/
typedef enum _GlgHandleInvisible
{
   GLG_SKIP_INVISIBLE_CELLS = 0,
   GLG_SHIFT_COLUMNS        = 1,
   GLG_SHIFT_ROWS           = 2,
   GLG_SHIFT_ROWS_COLUMNS   = 3,
   GLG_SHIFT_ALL_CELLS      = 7
} GlgHandleInvisible;

/* Choices of filling extra space left by invisible elements in a square series
   (rows are vertical if ColumnsFirst=YES).
*/
typedef enum _GlgFillSpace
{
   GLG_DONT_FILL_SPACE = 0,
   GLG_ADJUST_ROW_HEIGHT,
   GLG_ADJUST_COL_WIDTH,
   GLG_ADJUST_ROWS_AND_COLUMNS
} GlgFillSpace;

typedef enum _GlgKeepEditRatio
{
   GLG_DONT_KEEP_RATIO = 0,
   GLG_KEEP_RATIO,
   GLG_KEEP_SIZE
} GlgKeepEditRatio;
...
/* No XEvents in the GTK version, generate an error if XEvent is used in legacy
   code, which is then compiled for GTK. GXEvent should be used in the GTK 
   version. Using trace_data->event avoids declaring different event types for 
   GTK and X11 versions of the code.
*/
#define XEvent          XEvent_ERROR_USE_GXEvent_IN_GTK
...
#endif   /* MS_WINDOWS / GLG_GTK / X_WINDOWS */
...
/* Value options of the Plot's extended data attribute. */
typedef enum GlgExtDataFlag
{
   GLG_EXT_DATA_NONE = 0,   /* Disables extended data. */
   GLG_EXT_DATA_AUTO,       /* Enables extended data.  */
   GLG_EXT_DATA_LINE        /* Enables extended data and prioritizes 
                               line attributes over marker attributes. */
} GlgExtDataFlag;

/* Type of information stored in the extended data sample. */
...
typedef struct _GlgFindObjectsData
{
   ...
   CCONST char * object_name;
   CCONST char * resource_name;
   GlgObject object_id;
...
/*** Backward compatibility for use with GlgFindMatchingObjects, deprecated. ***/
#define GlgFindMatchingObjectsData   GlgFindObjectsData
...
#ifdef GLG_GTK
typedef union _GXEvent GXEvent;
#endif
...
#ifdef MS_WINDOWS       /*** Microsoft Windows ***/
   MSG * event;                    /* Event that triggered the callback. */
   HWND widget;                    /* Same as window. */
   HWND window;                    /* Window ID of the event viewport. */
   void * display;
   void * event1;
#elif defined GLG_GTK   /**** GTK ***/
   GXEvent * event;                /* Simulated event similar to X11 XEvent,
                                      that makes it possible to use the same code
                                      for both X11 and GTK trace callbacks. */
   GtkWidget * widget;             /* Widget ID of the event viewport. */
   GdkEvent * event1;              /* GDK event, may be NULL for some GXEvents.
                                    */
#else                   /*** X_WINDOWS ***/
...
GlgImport( void ) GlgDialogInitialDraw( GlgObject object, GlgObject parent );
GlgImport( void ) GlgSetupHierarchy( GlgObject object );
GlgImport( void ) GlgResetHierarchy( GlgObject object );
GlgImport( void ) GlgDialogSetupHierarchy( GlgObject object, GlgObject parent );
GlgImport( GlgBoolean ) GlgUpdate( GlgObject object );
GlgImport( GlgBoolean ) GlgReset( GlgObject object );

GlgImport( GlgBoolean ) GlgExportPostScript( GlgObject object,
                                             CCONST char * file,
                                             double x, double y, 
                                             ...
                                             GlgBoolean stretch,
                                             GlgBoolean center );
/*** Deprecated, use GlgExportPostScript. ***/
GlgImport( GlgBoolean ) GlgPrint( GlgObject object, CCONST char * file,
                                  double x, double y, 
                                  double width, double height,
                                  GlgBoolean portrait, GlgBoolean stretch );
#ifdef MS_WINDOWS
GlgImport( GlgBoolean ) GlgNativePrint( GlgObject object, HDC print_dc,
                                        double x, double y,
                                        double width, double height,
                                        GlgBoolean stretch, GlgBoolean center );
/*** Deprecated, use GlgNativePrint. ***/
...
#ifdef GLG_GTK
GlgImport( GlgBoolean ) GlgNativePrint( GlgObject object,
...
#else   /* !MS_WINDOWS */
# ifdef GLG_GTK
GlgImport( cairo_surface_t * ) GlgGenerateImage( GlgObject object,
                                                 CCONST char * res_name, 
                                                 cairo_surface_t * surface );
GlgImport( cairo_surface_t * ) GlgGenerateImageCustom( GlgObject object, 
                                                       CCONST char * res_name, 
                                                       cairo_surface_t * surface,
                                                       GlgLong x, GlgLong y, 
                                                       GlgLong width,
...
GlgImport( void ) GlgRecalculateObject( GlgObject object, CCONST char * res_name );
...
GlgImport( GlgBoolean ) GlgGetChartDataExtent( GlgObject object, 
                                               CCONST char * res_name, 
                                               GlgMinMax * min_max, 
                                               GlgBoolean pan_x,
                                               GlgBoolean visible_only );
/*** Deprecated, use GlgGetChartDataExtent ***/
...
/************* Function prototypes for ICC API (deprecated) *************/
...
GlgImport( GlgBoolean ) GlgSaveObject( GlgObject object, 
                                       CCONST char * filename );
GlgImport( void * ) GlgSerializeFull( GlgObject object, GlgLong * );
GlgImport( GlgBoolean ) GlgSerialize( GlgObject object,
                                      GlgBoolean (*func)( void * data,
                                                          GlgLong size,
                                                          void * user_data ),
                                      void * user_data );
...
GlgImport( void ) GlgSetEnd( GlgObject container );
GlgImport( GlgObject ) GlgIterateBack( GlgObject container );
GlgImport( GlgBoolean ) GlgIsAt( GlgObject container, GlgBoolean state );
...
GlgImport( GlgObject ) GlgQueryTags( GlgObject object, 
                                     GlgTagType tag_type_mask );
GlgImport( GlgBoolean ) GlgDeleteTags( GlgObject object, 
                                      GlgTagType tag_type_mask );
...
/* Deprecated, use GlgFindObjects. */
GlgImport( GlgBoolean ) GlgFindMatchingObjects( GlgObject object, 
                                                GlgFindMatchingObjectsData * data );
GlgImport( GlgBoolean ) GlgFindObjects( GlgObject object,
                                        GlgFindObjectsData * data );
GlgImport( void ) GlgTraverseObjects( GlgObject object, 
                                      GlgBoolean (*func)( GlgObject object,
                                                          void * data ),
                                      void * data );
GlgImport( void ) GlgTraverseObjectsExt( GlgObject object, 
                                         GlgBoolean (*enter_func)
                                           ( GlgObject object, void * data ),
                                         GlgBoolean (*exit_func)
                                           ( GlgObject object, void * data ),
                                         void * data );
...
GlgImport( GlgBoolean ) GlgWorldToScreen( GlgObject object, 
                                          ...
                                          GlgPoint * in_point, 
                                          GlgPoint * out_point );
GlgImport( GlgBoolean ) GlgTranslatePointOrigin( GlgObject from_viewport, 
...
GlgImport( GlgBoolean ) GlgPositionToValue( GlgObject object, 
                                            ...
                                            double * value );
#endif   /* !GLG_INTERNAL || GLG_INTERNAL_EXT_PROTO */
GlgImport( GlgDataSampleNode * ) GlgCreateDataSampleNode( GlgObject plot,
                                                          GlgBoolean extended );
GlgImport( void ) GlgFreeDataSampleNode( GlgObject plot,
                                         GlgDataSampleNode * node );
GlgImport( GlgDataSample * ) GlgGetNodeDataSample( GlgDataSampleNode * node );
#if !defined GLG_INTERNAL || defined GLG_INTERNAL_EXT_PROTO
GlgImport( void ) GlgAddDataSampleNode( GlgObject plot,
                                        GlgDataSampleNode * node,
                                        GlgBoolean quick_mode );
/*** Deprecated, doesn't work with charts that use chart cache.
     Use GlgCreateDataSampleNode. ***/
GlgImport( void * ) GlgCreateDataSample( GlgBoolean extended );
/*** Deprecated, doesn't work with charts that use chart cache.
     Use GlgAddDataSampleNode. ***/
...
GlgImport( void ) GlgSetMatrixData( GlgObject matrix, 
                                    GlgMatrixData * matrix_data );
GlgImport( GlgObject ) GlgAddAttachmentPoint( GlgObject object,
                                              double dx, double dy, double dz );
GlgImport( GlgBoolean ) GlgSetAttachmentMoveMode( GlgBoolean state );
GlgImport( GlgBoolean ) GlgEnableAttachmentPoints( GlgBoolean state );
#endif   /* !GLG_INTERNAL || GLG_INTERNAL_EXT_PROTO */
...
GlgImport( void ) GlgSetNBParent( GlgObject viewport, void * q_widget );
GlgImport( void * ) GlgGetNBParent( GlgObject viewport );
...
GlgImport( GlgBoolean ) GlgSetWindowReordering( GlgBoolean enabled );
#if !defined MS_WINDOWS && !defined GLG_GTK
...
GlgImport( GlgBoolean ) GlgXPrintDefaultError( Display *dpy, XErrorEvent *event,
                                               FILE * file );
#endif   /* !MS_WINDOWS && !GLG_GTK */
#ifdef GLG_GTK
GlgBoolean GlgSetEventPropagation( GlgBoolean state );
#endif
...
/* Returns a file path relative to the executable location using an allocated
   string. */
GlgImport( char * ) GlgGetRelativePath( CCONST char * exe_path, 
                                        CCONST char * filename );
/* An extended version of GlgGetRelativePath().
   If check_if_exists=True, and the file does not exist, returns NULL if 
...
/* Returns directory of the path as an allocated string.*/
GlgImport( char * ) GlgGetDir( CCONST char * path );
/* Returns NULL if the file is not found. Returns an allocated string. */
...
GlgImport( void ) GlgStopButtonRepeat( void );
GlgImport( GlgBoolean ) GlgSetCursorType( GlgObject object,
                                          CCONST char * res_name,
...
GlgImport( void ) GlgIHSetIParameter( GlgIH ih, CCONST char * name,
                                      GlgLong value );
GlgImport( void ) GlgIHSetPParameter( GlgIH ih, CCONST char * name,
                                      void * value );
GlgImport( void ) GlgIHSetSParameter( GlgIH ih, CCONST char * name,
                                      char * value );
GlgImport( void ) GlgIHSetOParameter( GlgIH ih, CCONST char * name,
                                      GlgObject value );
GlgImport( void ) GlgIHSetDParameter( GlgIH ih, CCONST char * name,
                                      double value );
GlgImport( void ) GlgIHSetOParameterFromD( GlgIH ih, CCONST char * name,
                                           double value );
GlgImport( void ) GlgIHSetOParameterFromG( GlgIH ih, CCONST char * name,
                                           double value1, double value2,
                                           double value3 );

GlgImport( GlgLong ) GlgIHGetIParameter( GlgIH ih, CCONST char * name );
GlgImport( void ) * GlgIHGetPParameter( GlgIH ih, CCONST char * name );
GlgImport( char ) * GlgIHGetSParameter( GlgIH ih, CCONST char * name );
GlgImport( GlgObject ) GlgIHGetOParameter( GlgIH ih, CCONST char * name );
GlgImport( double ) GlgIHGetDParameter( GlgIH ih, CCONST char * name );

GlgImport( GlgLong ) GlgIHGetOptIParameter( GlgIH ih, CCONST char * name, 
                                            GlgLong default_value );
GlgImport( void ) * GlgIHGetOptPParameter( GlgIH ih, CCONST char * name, 
                                           void * default_value );
GlgImport( char ) * GlgIHGetOptSParameter( GlgIH ih, CCONST char * name, 
                                           char * default_value );
GlgImport( GlgObject ) GlgIHGetOptOParameter( GlgIH ih, CCONST char * name, 
                                              GlgObject default_value );
GlgImport( double ) GlgIHGetOptDParameter( GlgIH ih, CCONST char * name, 
                                           double default_value );

GlgImport( void ) GlgIHChangeIParameter( GlgIH ih, CCONST char * name,
                                         GlgLong value );
GlgImport( void ) GlgIHChangePParameter( GlgIH ih, CCONST char * name,
                                         void * value );
GlgImport( void ) GlgIHChangeSParameter( GlgIH ih, CCONST char * name,
                                         char * value );
GlgImport( void ) GlgIHChangeOParameter( GlgIH ih, CCONST char * name, 
                                         GlgObject value );
GlgImport( void ) GlgIHChangeDParameter( GlgIH ih, CCONST char * name,
                                         double value );


#if defined GLG_GTK && !defined GLG_INTERNAL
...
typedef struct
{
   GXEventType type;      /* KeyPress or KeyRelease */
   int x, y;		  /* Coordinates in event widget. */
   int x_root, y_root;	  /* Coordinates relative to the GTK top level window. */
   unsigned int keycode;  /* The key that was pressed or released.
                             See gdk/gdkkeysyms.h a list of GDK key codes. */
} GXKeyEvent;

typedef struct
{
   GXEventType type;      /* ButtonPress or ButtonRelease */
   int x, y;		  /* Coordinates in event widget. */
   int x_root, y_root;	  /* Coordinates relative to the GTK top level window. */
...
typedef struct
{
   GXEventType type;      /* LeaveNotify or EnterNotify */
   int x, y;		  /* Coordinates in event widget. */
   int x_root, y_root;	  /* Coordinates relative to the GTK top level window. */
} GXCrossingEvent;
...
typedef struct
{
   GXEventType type;      /* Expose, generated when the widget is drawn. */
   int x, y;              /* Origin of the area to be drawn. */
   int width, height;     /* Size of the area to be drawn. */
} GXExposeEvent;

typedef struct
{
   GXEventType type;      /* VisibilityNotify */
   int state;		  /* 0 if visible, 2 if invisible. */
} GXVisibilityEvent;

typedef struct
{
   GXEventType type;      /* UnmapNotify */
} GXUnmapEvent;

typedef struct
{
   GXEventType type;      /* MapNotify */
} GXMapEvent;

typedef struct
{
   GXEventType type;      /* ConfigureNotify, generated when the widget is 
                             resized. */
   int x, y;              /* Widget origin relatively to the parent widget. */
   int width, height;     /* Widget size. For drawing area widgets, it's an
                             inner size inside the widget's border. */
} GXConfigureEvent;
