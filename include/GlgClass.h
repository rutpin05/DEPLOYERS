...
/* Define one of the following macros to specify the API to be used:
   ...
   Define GLG_ENABLE_X11_WRAPPER to use C++ bindings with the legacy X11 version
   of the GLG library on Linux/Unix.
*/

#include <stdio.h>
#include "GlgApi.h"

#if !defined _WINDOWS && defined GLG_ENABLE_X11_WRAPPER
...
class GlgObjectC
{
 public:
   ...
   // Update/Sync viewports. 
   GlgBoolean Sync( void );
   GlgBoolean Update( void );
   
   // Printing
   GlgBoolean ExportPostScript( SCONST char * filename="out.ps",
   ...
   cairo_surface_t * GenerateImage( SCONST char * res_name = NULL, 
                                    cairo_surface_t * surface = (cairo_surface_t *)0 );
   cairo_surface_t * GenerateImageCustom( SCONST char * res_name,
   ...
   GlgBoolean GetChartDataExtent( SCONST char * res_name, 
                                  GlgMinMax * min_max, GlgBoolean pan_x,
...
// GlgWrapperC
//
// C++ wrapper around the legacy X11 version of the GlgWrapper widget.
// It is a subclass of a GlgObjectC and inherits GLG API and GLG Extended API
// from it.
//
