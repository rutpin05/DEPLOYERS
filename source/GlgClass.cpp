// Need to emulate this even on Unix: Windows does not allow ifdef'ing it
#include "stdafx.h"
...
void GlgObjectC::DialogInitialDraw( GlgObjectC& parent )
{
   if( glg_obj )
     GlgDialogInitialDraw( glg_obj, parent.glg_obj );
}
...
void GlgObjectC::DialogSetupHierarchy( GlgObjectC& parent )
{
   if( glg_obj )
     GlgDialogSetupHierarchy( glg_obj, parent.glg_obj );
}
...
GlgBoolean GlgObjectC::ExportPostScript( SCONST char * filename,
                                         double x, double y,
                                         ...
                                         GlgBoolean center )
{
   if( !glg_obj )
     return False;
   return
     GlgExportPostScript( glg_obj, STRING_CAST filename, 
                          x, y, width, height, portrait, stretch, center );
}
                              ...
                              GlgBoolean center )
{
   if( !glg_obj )
     return False;
   return
     GlgNativePrint( glg_obj, dc->GetSafeHdc(), x, y, width, height,
                     stretch, center );
}

void GlgObjectC::OnPrint( CDC * dc )
{
   GlgNativePrint( glg_obj, dc->GetSafeHdc(), 0., 0., -1., -1., 0, 0 );
}
...
cairo_surface_t * GlgObjectC::GenerateImage( SCONST char * res_name,
...
HBITMAP GlgObjectC::GenerateImageCustom( SCONST char * res_name, HBITMAP pixmap,
                                         ...
                                         GlgLong gap )
#elif defined GLG_GTK
cairo_surface_t * GlgObjectC::GenerateImageCustom( SCONST char * res_name,
                                                   cairo_surface_t * pixmap,
                                                   GlgLong x, GlgLong y, 
                                                   GlgLong width,
                                                   GlgLong height, GlgLong gap )
#else
                                      ...
                                      GlgMinMax * min_max, GlgBoolean pan_x )
{
   if( !glg_obj )
     return False;   
   return GlgGetChartDataExtent( glg_obj, STRING_CAST res_name, min_max, pan_x,
                                 false );
}

GlgBoolean GlgObjectC::GetChartDataExtent( SCONST char * res_name, 
                                           GlgMinMax * min_max,
                                           GlgBoolean pan_x,
